 ...
 */
const generateCards = (locals, tasks, users, groups, assignee) => {
  const cardsConfig = require('../config.json')
  delete require.cache[require.resolve('../config.json')]
  const linearizedTickets = linearizeData(locals, tasks, users, groups, assignee)
  const requestMap = {
 ...
 */
const linearizeData = (locals, tasks, users, groups, assignee) => {
  /**
  ...
  return tasks.map(r => {
    ...
    const dueDate = r.due_at ? r.due_at : ''
