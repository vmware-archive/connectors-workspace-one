...
class HubCoupaControllerTestBase extends ControllerTestsBase {
    ...
    WebTestClient.ResponseSpec approveRequestWithComment(String authHeader) {
        return approveRequest(authHeader, "/api/approve/182964?comment=Approved");
    }

    WebTestClient.ResponseSpec approveRequestWithoutComment(String authHeader) {
        return approveRequest(authHeader, "/api/approve/182964");
    }

    private WebTestClient.ResponseSpec approveRequest(String authHeader, String uri) {
    ...
    WebTestClient.ResponseSpec rejectRequestWithoutComment(String authHeader) {
        String uri = "/api/decline/182964";
    ...
    void fetchAttachment(String authHeader) throws IOException {
        byte[] result = getAttachment(authHeader)
                .expectStatus().isOk()
                .expectHeader().contentDisposition(ContentDisposition.parse("attachment; filename=\"response.png\""))
    ...
    void mockApproveActionsWithComment(String serviceCredential) throws Exception {
        mockRequisitionDetails(serviceCredential);
        mockApproveAction(serviceCredential, "/api/approvals/6609559/approve?reason=Saurabh%20Deswal%20has%20commented%3A%20Approved");
    }

    void mockApproveActionsWithoutComment(String serviceCredential) throws Exception {
        mockRequisitionDetails(serviceCredential);
        mockApproveAction(serviceCredential, "/api/approvals/6609559/approve");
    }
    ...
    void mockApproveAction(String serviceCredential, String uri) {
        mockBackend.expect(requestTo(uri))
    ...
    void mockRejectAction(String serviceCredential) {
        mockBackend.expect(requestTo("/api/approvals/6609559/reject?reason=Saurabh%20Deswal%20has%20commented%3A%20Declined"))
                .andExpect(method(PUT))
                .andExpect(header(ACCEPT, APPLICATION_JSON_VALUE))
                .andExpect(header(AUTHORIZATION_HEADER_NAME, serviceCredential))
                .andRespond(withSuccess());
