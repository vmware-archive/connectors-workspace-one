...
public class ArgumentUtilsTest {
    ...
    public void whenInvalidBasicConnectorInputsProvided(final String baseUrl, final String workdayAuth, final String email) {
        assertThatExceptionOfType(IllegalArgumentException.class)
            .isThrownBy(() -> ArgumentUtils.checkBasicConnectorArgumentsNotBlank(baseUrl, workdayAuth, email));
    }

    @Test public void exceptionNotThrownWhenValidBasicConnectorInputsProvided() {
        assertThatCode(() -> ArgumentUtils.checkBasicConnectorArgumentsNotBlank(BASE_URL, WORKDAY_TOKEN, EMAIL)).doesNotThrowAnyException();
    }
    ...
    private static List<String> blankInputArguments() {
        return Lists.newArrayList(NO_EMAIL, EMPTY_EMAIL, SPACE);
    }
    ...
    public void exceptionThrownWhenStringInputIsBlankForCheckArgumentNotBlank(final String stringInput) {
        assertThatExceptionOfType(IllegalArgumentException.class)
            .isThrownBy(() -> ArgumentUtils.checkArgumentNotBlank(stringInput, EMAIL_LABEL));
    }

    @Test public void exceptionNotThrownWhenStringInputIsNotBlankForCheckArgumentNotBlank() {
        assertThatCode(() -> ArgumentUtils.checkArgumentNotBlank(EMAIL, EMAIL_LABEL)).doesNotThrowAnyException();
    }
}
