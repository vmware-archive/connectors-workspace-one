...
const { getUserInfo, getUserTickets, getGroups, getTicketComments, addTicketComment, updateTicketStatus, getUsers } = require('../../services/zendesk-ticket-service')
...
describe('Service API tests', () => {
    ...
    it('Get userInfo by emailId', async () => {
      nock('https://vmware.zendesk.com')
        .get('/api/v2/search.json?query=type:user srini.gargeya@vmware.com')
        .reply(200, userInfoMockResp)
      const response = await getUserInfo({
        connectorAuthorization: 'Bearer connectorAuthorizationValue',
        apiBaseUrl: 'https://vmware.zendesk.com',
  ...
  describe('addTicketComment', () => {
    it('addTicketComment happy path', async () => {
      nock('https://vmware.zendesk.com')
        .put('/api/v2/tickets/6.json', {
          ticket: {
            comment: "We are working on this as a prioirty"
          }
        })
        .reply(200, addCommentAPIResp)
      const response = await addTicketComment({
        connectorAuthorization: 'Bearer connectorAuthorizationValue',
        apiBaseUrl: 'https://vmware.zendesk.com',
        mfJwt: { email: 'srini.gargeya@vmware.com' }
      }, 6, 'We are working on this as a prioirty')
      expect(typeof response).to.equal('object')
      expect(response.ticket.id).to.equal(6)
      expect(response.ticket.description).to.equal('Printer not working for last one week')
      expect(response.ticket.status).to.equal('open')
      expect(response.audit.events[0].body).to.equal('We are working on this as a prioirty')
    })

    it('addTicketComment in network error', async () => {
      nock('https://vmware.zendesk.com')
        .put('/api/v2/tickets/6.json', {
          ticket: {
            comment: "We are working on this as a prioirty"
      ...
      try {
        ...
        sinon.assert.fail('addTicketComment should throw exception. so this path should not be reachable')
      } catch (err) {
        const error = JSON.parse(err.message)
        expect(error.statusCode).to.equal(401)
      }
