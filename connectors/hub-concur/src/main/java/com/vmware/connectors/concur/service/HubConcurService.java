...
public class HubConcurService {
	...
	@Autowired
	public HubConcurService(WebClient rest, CardTextAccessor cardTextAccessor,
	...
	private Mono<ExpenseReportResponse> fetchRequestData(String baseUrl, String reportId) {
		...
		return rest.get().uri(baseUrl + "/api/expense/expensereport/v2.0/report/{reportId}", reportId)
				.header(AUTHORIZATION, serviceAccountAuthHeader).accept(APPLICATION_JSON).retrieve()
				.bodyToMono(ExpenseReportResponse.class);
	}

	private Card makeCards(String routingPrefix, Locale locale, ExpenseReportResponse report,
			HttpServletRequest request) {
		String reportId = report.getReportID();
		String reportName = report.getReportName();

		logger.debug("makeCard called: routingPrefix={}, reportId={}, reportName={}", routingPrefix, reportId,
				reportName);
		...
		Card.Builder builder = new Card.Builder().setName("Concur")
				...
				.setBody(new CardBody.Builder().addField(makeSubmissionDateField(locale, report))
						.addField(makeRequestedByField(locale, report)).addField(makeCostCenterField(locale, report))
						.addField(makeExpenseAmountField(locale, report)).build())
	...
	private CardBodyField makeRequestedByField(Locale locale, ExpenseReportResponse report) {
		return new CardBodyField.Builder().setType(CardBodyFieldType.GENERAL)
				...
				.setDescription(report.getEmployeeName()).build();
	}
	...
	private CardBodyField makeCostCenterField(Locale locale, ExpenseReportResponse report) {
		return new CardBodyField.Builder().setType(CardBodyFieldType.GENERAL)
				...
				.setDescription(report.getCostCenter()).build();
	}
	...
	private CardBodyField makeSubmissionDateField(Locale locale, ExpenseReportResponse report) {
		return new CardBodyField.Builder().setType(CardBodyFieldType.GENERAL)
				...
				.setDescription(report.getSubmitDate()).build();
	}
	...
	private CardAction makeApproveAction(String routingPrefix, Locale locale, String reportId) {
		return new CardAction.Builder().setActionKey(CardActionKey.USER_INPUT)
				...
				.addUserInputField(new CardActionInputField.Builder().setFormat("textarea")
						...
						.setLabel(cardTextAccessor.getMessage("hub.concur.approve.comment.label", locale)).build())
				.build();
	}
	...
	private CardAction makeDeclineAction(String routingPrefix, Locale locale, String reportId) {
		return new CardAction.Builder().setActionKey(CardActionKey.USER_INPUT)
				...
				.addUserInputField(new CardActionInputField.Builder().setFormat("textarea")
						...
						.setLabel(cardTextAccessor.getMessage("hub.concur.decline.reason.label", locale)).build())
				.build();
	}
