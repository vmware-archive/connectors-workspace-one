...
describe('jira_rest tests:', () => {
  ...
  before(() => {
    if (!server) {
      ...
      try {
        ...
        process.env.token_public_key_url = 'http://localhost:10002/public-key'
  ...
  describe('auth tests:', function () {
    it('envPublicKeyURL should return the URL from the env', function () {
      ...
      expect(url === process.env.token_public_key_url, `expected ${process.env.token_public_key_url} got ${url}`)
    })
