...
cp files/jira-connector.service /etc/systemd/system/
cp files/logback.xml /etc/opt/vmware/connectors/
cp files/jira-connector.jar /opt/vmware/connectors/


sed s~{TENANT_PUBLIC_KEY_URL}~$TENANT_PUBLIC_KEY_URL~ files/jira-connector.service > /etc/systemd/system/jira-connector.service

systemctl daemon-reload
systemctl start jira-connector
systemctl enable jira-connector